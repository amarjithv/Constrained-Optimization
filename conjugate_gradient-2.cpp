...
vector<double> first_deriv( vector<double> y, int fno) // function to calculate derivative
{
        vector<double> fd_x;
        ...
        for(int i=0; i<y.size(); i++){
                ...
                if (abs_value(y[i])>0.01)
                        ...
                        delx=0.0001;
                y[i]=y[i]+delx;
                fx1=obj_fun(y, fno); // f(x)+delx
                y[i]=y[i]-2*delx;
                fx2=obj_fun(y, fno); // f(x)-delx
		func_evals+=2;
                y[i]=y[i]+delx;
...
vector<double> direction(vector<double> dir_prev, vector<double> fir_der_cur, vector<double> fir_der_prev)//update the direction
{
        vector<double> fir_der;
        double multiplier = pow(dotpro(fir_der_cur),2)/pow(dotpro(fir_der_prev),2); // ratio of squares of grads
        vector<double> neg_der;
        for(int i=0; i<dir_prev.size(); i++){
                neg_der.push_back(-fir_der_cur[i]); // negative of grad of current point.
        }
        vector<double> sec_term = multiplying(dir_prev, multiplier);
        vector<double> dir = adding(neg_der, sec_term); //the new search direction
        return dir;
}

double dotpro(vector<double> x) // function to calculate the absolute value
{
        int n = x.size();
        double sum=0;
        for(int i=0; i<n; i++){
...
double dotpro2(vector<double> x, vector<double> y) //function to find the angle between two directions.
{
        ...
        return acos(sum3/(sqrt(sum1)*sqrt(sum2))); // finding cos inverse.
